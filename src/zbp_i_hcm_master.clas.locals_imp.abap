CLASS lcl_buffer DEFINITION.
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_buffer_master.
             INCLUDE TYPE zhc_master_8712 AS data.
...
CLASS lhc_HCMMaster IMPLEMENTATION.
  ...
  METHOD update.
    ...
    LOOP AT entities INTO DATA(ls_entities).
      ...
      SELECT SINGLE * FROM zhc_master_8712
      ...
      INSERT VALUE #(
          ...
          data = VALUE #( e_number = ls_entities-%data-e_number
                          ...
                          start_date = COND #(
                           ...
                           end_date = COND #(
                              WHEN ls_entities-%control-end_date EQ if_abap_behv=>mk-on
                              THEN ls_entities-%data-end_date
                              ELSE ls_ddbb-end_date )

                           email = COND #(
                              WHEN ls_entities-%control-email EQ if_abap_behv=>mk-on
                              THEN ls_entities-%data-email
                              ELSE ls_ddbb-email )


                           m_number = COND #(
                              WHEN ls_entities-%control-m_number EQ if_abap_behv=>mk-on
                              THEN ls_entities-%data-m_number
                              ELSE ls_ddbb-m_number )

                           m_name = COND #(
                              WHEN ls_entities-%control-m_name EQ if_abap_behv=>mk-on
...
CLASS lsc_HCMMaster IMPLEMENTATION.
  ...
  METHOD save.
    ...
    DATA:
      lt_data_created TYPE STANDARD TABLE OF zhc_master_8712,
      lt_data_updated TYPE STANDARD TABLE OF zhc_master_8712,
      lt_data_deleted TYPE STANDARD TABLE OF zhc_master_8712.

    lt_data_created
        = VALUE #( FOR <row> IN lcl_buffer=>mt_buffer_master
                    WHERE ( flag = lcl_buffer=>created ) ( <row>-data ) ).

    IF NOT lt_data_created IS INITIAL.
      INSERT zhc_master_8712 FROM TABLE @lt_data_created.
    ENDIF.
    ...
    IF NOT lt_data_updated IS INITIAL.
      UPDATE zhc_master_8712 FROM TABLE @lt_data_updated.
    ENDIF.
    ...
    IF NOT lt_data_deleted IS INITIAL.
      DELETE zhc_master_8712 FROM TABLE @lt_data_deleted.
    ENDIF.
